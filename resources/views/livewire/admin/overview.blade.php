<div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <!-- Total Users -->
        <div class="bg-white rounded-sm border border-hairline p-6 relative overflow-hidden group hover:border-amber/30 transition-colors">
            <div class="absolute top-0 right-0 w-24 h-24 bg-gradient-to-br from-amber/5 to-transparent rounded-bl-full -mr-4 -mt-4 opacity-50 group-hover:scale-110 transition-transform duration-500"></div>
            <div class="flex items-center gap-4 mb-4">
                <div class="w-10 h-10 rounded-sm bg-paper border border-hairline flex items-center justify-center shrink-0">
            ...
            <div class="flex items-baseline gap-2">
                <span class="font-mono text-3xl font-bold text-ink tracking-tight">{{ number_format($totalUsers, 0, ',', '.') }}</span>
            </div>
        </div>
        ...
        <div class="bg-white rounded-sm border border-hairline p-6 relative overflow-hidden group hover:border-amber/30 transition-colors">
            ...
            <div class="flex items-baseline gap-2">
                <span class="font-mono text-3xl font-bold text-ink tracking-tight">{{ number_format($activeProUsers, 0, ',', '.') }}</span>
            </div>
        </div>
        ...
        <div class="bg-white rounded-sm border border-hairline p-6 relative overflow-hidden group hover:border-amber/30 transition-colors">
            ...
            <div class="flex items-baseline gap-2">
                ...
                <span class="font-mono text-2xl font-bold text-ink tracking-tight">{{ number_format($totalRevenue, 0, ',', '.') }}</span>
            </div>
        </div>
        ...
        <div class="bg-white rounded-sm border border-hairline p-6 relative overflow-hidden group hover:border-amber/30 transition-colors">
            ...
            <div class="flex items-baseline gap-2">
                <span class="text-sm text-ink-muted font-medium">Rp</span>
                <span class="font-mono text-2xl font-bold text-ink tracking-tight">{{ number_format($revenueThisMonth, 0, ',', '.') }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white border border-hairline rounded-sm">
        <div class="px-6 py-5 border-b border-hairline flex items-center justify-between">
            ...
            <a href="{{ route('admin.transactions') }}" wire:navigate class="text-sm text-amber hover:text-amber/80 font-medium transition-colors">Lihat Semua</a>
        </div>
