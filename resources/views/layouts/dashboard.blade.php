...
<body class="bg-paper text-ink font-sans antialiased">
    <div x-data="{ sidebarOpen: true }" class="flex h-screen overflow-hidden print:h-auto print:overflow-visible print:block">
        ...
        <aside :class="sidebarOpen ? 'w-64' : 'w-21'"
            class="print:hidden bg-ink text-slate-300 flex flex-col h-full shrink-0 transition-all duration-300 overflow-hidden relative">
            <div class="px-6 py-5 border-b border-white/10 shrink-0 flex items-center h-18.25">
                <a href="{{ route('dashboard') }}" wire:navigate
                    class="font-display font-bold text-white text-lg tracking-tight flex items-center gap-3 overflow-hidden whitespace-nowrap">
                ...
                <div x-data="{ toolsOpen: {{ request()->is('tool/*') ? 'true' : 'false' }} }" class="mt-2 border-t border-white/10 pt-4">
                    <button @click="sidebarOpen ? toolsOpen = !toolsOpen : (sidebarOpen = true, toolsOpen = true)"
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <span x-show="sidebarOpen" x-transition.opacity.duration.300ms
                                class="whitespace-nowrap font-medium">Kumpulan Tools</span>
                        </div>
                        <x-lucide-chevron-down x-show="sidebarOpen"
                            class="w-4 h-4 shrink-0 transition-transform duration-200"
                            x-bind:class="toolsOpen ? 'rotate-180' : ''" />
                    </button>

                    <div x-show="toolsOpen && sidebarOpen" x-transition:enter="transition ease-out duration-200"
                        x-transition:enter-start="opacity-0 -translate-y-2"
                        x-transition:enter-end="opacity-100 translate-y-0"
                        x-transition:leave="transition ease-in duration-150"
                        x-transition:leave-start="opacity-100 translate-y-0"
                        x-transition:leave-end="opacity-0 -translate-y-2" class="bg-black/20 py-2 mt-1 space-y-1"
                        x-cloak>
                        ...
                        @foreach (config('tools') as $tool)
                            <a href="{{ route('tool', $tool['slug']) }}" wire:navigate title="{{ $tool['name'] }}"
                                ...
                                <div class="w-5 flex justify-center shrink-0">
                                    <div
                                        class="w-1.5 h-1.5 rounded-full {{ request()->is('tool/' . $tool['slug']) ? 'bg-amber' : 'bg-slate-600' }}">
                                    </div>
                                </div>
            ...
            <div class="py-4 border-t border-white/10 shrink-0 overflow-hidden">
                <a href="{{ route('profile') }}" wire:navigate title="Profil Saya"
                    ...
                    <span x-show="sidebarOpen" x-transition.opacity.duration.300ms class="whitespace-nowrap">Profil
                        Saya</span>
                </a>
        ...
        <div class="flex-1 flex flex-col h-full overflow-hidden print:h-auto print:overflow-visible print:block">
            <!-- Topbar -->
            <header
                class="print:hidden bg-paper border-b border-hairline px-8 py-4 flex justify-between items-center shrink-0 h-18.25">
                <div class="flex items-center gap-4">
                    <button @click="sidebarOpen = !sidebarOpen"
                        class="cursor-pointer text-ink-muted hover:text-ink transition-colors p-1 -ml-2 rounded-sm focus:outline-none focus:ring-2 focus:ring-amber/50">
                ...
                <div x-data="{
                    ...
                    <button @click="dropdownOpen = !dropdownOpen" @click.away="dropdownOpen = false"
                        ...
                        <div
                            class="w-8 h-8 rounded-full bg-amber/15 border border-amber/40 text-amber font-mono text-sm flex items-center justify-center overflow-hidden">
                            <img x-show="photoUrl" :src="photoUrl" :alt="name"
                                class="w-full h-full object-cover" style="display: none;" x-cloak>
                            <span x-show="!photoUrl" x-text="initials()"></span>
                        </div>
                        <x-lucide-chevron-down
                            class="w-4 h-4 text-ink-muted transition-transform duration-200 hidden sm:block"
                    ...
                    <div x-show="dropdownOpen" x-transition:enter="transition ease-out duration-100"
                            ...
                            @else
                                <div class="flex items-center justify-between">
                                    ...
                                    <a href="{{ route('pricing') }}"
                                        class="text-[10px] bg-amber text-ink px-2 py-0.5 rounded-sm hover:bg-amber/90">Upgrade</a>
                                </div>
                            @endif
                        </div>
                        ...
                        <a href="{{ route('profile') }}" wire:navigate
                            class="flex items-center gap-3 px-4 py-2 text-sm text-ink hover:bg-paper transition-colors">
                        ...
                        @if (auth()->user()->is_admin)
                            <a href="{{ route('admin.overview') }}" wire:navigate
                                class="flex items-center gap-3 px-4 py-2 text-sm text-ink hover:bg-paper transition-colors border-t border-hairline border-b">
            ...
            <!-- Main content -->
            <main
                class="flex-1 overflow-y-auto bg-paper flex flex-col print:overflow-visible print:bg-white print:block">
