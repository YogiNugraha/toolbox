...
<head>
    ...
    @endphp
    <title>Pembayaran Berhasil - {{ $siteName }}</title>
</head>
                    ...
                    <tr>
                        <td style="padding: 32px;">
                            ...
                            <div style="text-align: center; margin-bottom: 20px;">
                                <span style="display: inline-block; background-color: #dcfce7; color: #15803d; font-size: 11px; font-weight: 800; padding: 4px 14px; border-radius: 9999px; text-transform: uppercase; letter-spacing: 0.5px;">
                                    ✓ Pembayaran Berhasil & Akun Aktif
                                </span>
                            </div>

                            <h2 style="margin: 0 0 12px 0; font-size: 18px; font-weight: 700; color: #0f172a; text-align: center;">
                                Terima Kasih, {{ $subscription->user->name }}!
                            </h2>
                            <p style="margin: 0 0 20px 0; font-size: 13px; color: #475569; text-align: center; line-height: 1.6;">
                                Transaksi pembayaran langganan <strong>{{ $siteName }} {{ $subscription->plan->name ?? ucfirst($subscription->plan_slug) }}</strong> telah terverifikasi. Akun Anda kini memiliki akses penuh tanpa batas ke seluruh fitur unggulan!
                            </p>
                                    ...
                                    <td style="padding: 16px;">
                                        <table width="100%" cellpadding="0" cellspacing="0" style="font-size: 12px;">
                                            <tr>
                                                ...
                                                <td align="right" style="font-weight: 700; color: #0f172a; padding-bottom: 8px;">#{{ $subscription->midtrans_order_id }}</td>
                                            </tr>
                                            <tr>
                                                <td style="color: #64748b; padding-bottom: 8px;">Paket</td>
                                                <td align="right" style="font-weight: 700; color: #4f46e5; padding-bottom: 8px;">{{ $subscription->plan->name ?? ucfirst($subscription->plan_slug) }}</td>
                                            </tr>
                                            <tr>
                                                <td style="color: #64748b; padding-bottom: 8px;">Tanggal Transaksi</td>
                                                <td align="right" style="font-weight: 600; color: #0f172a; padding-bottom: 8px;">{{ $subscription->created_at->translatedFormat('d F Y, H:i') }}</td>
                                            </tr>
                                            <tr>
                                                <td style="color: #64748b; padding-bottom: 8px;">Masa Aktif Hingga</td>
                                                <td align="right" style="font-weight: 600; color: #0f172a; padding-bottom: 8px;">{{ $subscription->expires_at ? $subscription->expires_at->translatedFormat('d F Y') : '-' }}</td>
                                            </tr>
                                            <tr style="border-top: 1px solid #e2e8f0;">
                                                <td style="padding-top: 10px; font-weight: 700; color: #0f172a; font-size: 13px;">Total Pembayaran</td>
                                                <td align="right" style="padding-top: 10px; font-weight: 800; color: #4f46e5; font-size: 15px;">Rp {{ number_format($subscription->amount, 0, ',', '.') }}</td>
                                            </tr>
                                        </table>
                                    </td>
                                </tr>
                            ...
                            <p style="margin: 0 0 24px 0; font-size: 12px; color: #64748b; text-align: center; background-color: #eff6ff; border: 1px solid #dbeafe; padding: 10px; border-radius: 8px;">
                                Salinan <strong>PDF Invoice resmi</strong> telah kami lampirkan bersama email ini untuk keperluan pembukuan Anda.
                            </p>
                            ...
                            <div style="text-align: center; margin-bottom: 24px;">
                                <a href="{{ route('dashboard') }}" style="background-color: #4f46e5; color: #ffffff; font-size: 13px; font-weight: 700; text-decoration: none; padding: 12px 32px; border-radius: 9999px; display: inline-block; box-shadow: 0 4px 10px rgba(79, 70, 229, 0.25);">
                                    Mulai Gunakan Fitur Pro &rarr;
                                </a>
                            </div>

                            <p style="margin: 0; font-size: 12px; color: #94a3b8; text-align: center; line-height: 1.5;">
                                Ada pertanyaan atau butuh bantuan? Hubungi tim kami di <a href="mailto:{{ $supportEmail }}" style="color: #4f46e5; text-decoration: none; font-weight: 600;">{{ $supportEmail }}</a>
                            </p>
                        </td>
                    </tr>
